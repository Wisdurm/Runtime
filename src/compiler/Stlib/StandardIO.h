...
namespace rt
{
	...
	objectOrValue FileCreate(std::vector<objectOrValue>& args, SymbolTable* symtab, ArgState& argState)
	{
        ...
        if (args.size() > 1)
	    {
            ...
            auto arg1 = getValue(args.at(1), symtab, argState);
            std::string path;
	...
	objectOrValue FileOpen(std::vector<objectOrValue>& args, SymbolTable* symtab, ArgState& argState)
	{
        ...
        if (args.size() > 0)
	    {
            ...
            // Get path
            std::string path;
            auto p = getValue(*file->getMember(std::string("path")), symtab, argState); // Constructor jank; the ghost of ast::value
	...
	objectOrValue FileAppendLine(std::vector<objectOrValue>& args, SymbolTable* symtab, ArgState& argState)
	{
        ...
        if (args.size() > 1)
	    {
            ...
            if (not f->is_open())
				return giveException("File failed to open");
            // Get text to add
            auto write = getValue(args.at(1), symtab, argState);
	...
	objectOrValue FileWrite(std::vector<objectOrValue>& args, SymbolTable* symtab, ArgState& argState)
	{
        ...
        if (args.size() > 1)
	    {
            ...
            if (not f->is_open())
				return giveException("File failed to open");
			// Get file position
			auto v = getValue(*file->getMember(std::string("pointer")), symtab, argState);
	...
	objectOrValue FileRead(std::vector<objectOrValue>& args, SymbolTable* symtab, ArgState& argState)
	{
        ...
        if (args.size() > 0)
	    {
			...
			f->seekg(pos);
			// Get data amount
			auto a = getValue(args.at(1), symtab, argState);
